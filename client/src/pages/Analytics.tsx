			...
			<div className="w-full flex">
				{(data.data[0].meeting.host as IUser)?._id === user?._id && (
					<div className={` w-[320px] h-full  py-8 pl-8 pr-0 `}>
						<div className={`w-full `} style={{ height: "calc(100vh - 60px)" }}>
							<h3
								className={`pb-4 text-sm ${theme === "dark" ? "text-white-800" : "tetx-black-600"}`}
							>
								Participants
							</h3>
				...
				<div
					className={`flex-1 h-full  py-8 pr-8 ${
						(data.data[0].meeting.host as IUser)?._id === user?._id
							? `pl-4 border-l ${theme === "dark" ? "border-transparent-400" : "border-gray-800"}`
							: "pl-8"
					}`}
				>
					<h3
					...
					<div className={`w-full h-full space-y-3 overflow-auto`}>
						...
						{<MuteTime selectedStat={selectedStat} />}
					</div>
				</div>
			</div>
		)
