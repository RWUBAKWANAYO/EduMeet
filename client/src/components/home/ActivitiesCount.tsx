...
import { useCountMeetings } from "../meetings/useMeetings";
import { useCountMeetingMessages } from "../meeting_room/useMeetingRoom";

export const ActivitiesCount: React.FC = () => {
	const { theme } = useContext(UIContext);
	const { isLoading: invitationsLoading, data: invitationsData } = useCountInvitations();
	const { isLoading: meetingsLoading, data: meetingsData } = useCountMeetings();
	...
	return (
		...
		>
			{invitationsLoading && meetingsLoading && messagesLoading ? (
				...
				<>
					...
					<div className="w-full grid grid-cols-2  gap-2">
						...
						<ActivityCard title="Recorded Meetings" count={115} />
						{messagesData?.data && (
