...
export const HeadLine: React.FC = () => {
  ...
  const meetingLink = `${process.env.REACT_APP_PROJECT_HOST}/meeting/${meetings[0].session_id}`;
  return (
      ...
      <div>
        ...
        >
          {meetings[0].start_time &&
            moment(meetings[0].start_time).format("MMM Do, YYYY | hh:mm A")}
        </p>
      </div>
      <div className="flex items-center space-x-4">
        <div className="w-36 relative h-10 flex">
          {meetings[0].participants.slice(0, 3).map((participant, index: number) => (
            <RenderAvatar
          ...
          {meetings[0].participants.length > 0 && (
            <button
